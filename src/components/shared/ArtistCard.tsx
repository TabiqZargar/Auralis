    ...
    open(e.clientX, e.clientY, [
      {
        ...
        onClick: () => {
          if (artist.topTracks.length > 0) {
            ...
            q.setQueue(artist.topTracks.map(s => ({ song: s, addedBy: "user", addedAt: new Date().toISOString(), position: 0 })));
            p.setCurrentSong(artist.topTracks[0]!);
          }
        },
      },
      {
        ...
        onClick: () => {
          const q = useQueueStore.getState();
          artist.topTracks.forEach((track) => q.addToQueue({ song: track, addedBy: "user", addedAt: new Date().toISOString(), position: 0 }));
          artist.albums.forEach((album) =>
            album.songs.forEach((song) => q.addToQueue({ song, addedBy: "user", addedAt: new Date().toISOString(), position: 0 })),
          );
