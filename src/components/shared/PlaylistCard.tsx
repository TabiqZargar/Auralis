    ...
    const items: Parameters<typeof open>[2] = [
      {
        ...
        onClick: () => {
          if (playlist.songs.length > 0) {
            ...
            q.setQueue(playlist.songs.map(s => ({ song: s, addedBy: "user", addedAt: new Date().toISOString(), position: 0 })));
            p.setCurrentSong(playlist.songs[0]!);
          }
        },
      },
      {
        ...
        onClick: () => {
          const q = useQueueStore.getState();
          playlist.songs.forEach((song) => q.addToQueue({ song, addedBy: "user", addedAt: new Date().toISOString(), position: 0 }));
        ...
        ? [
            ...
            },
            {
              ...
              onClick: () => {
                ...
                const original = lib.playlists.find(p => p.id === playlist.id);
                if (original) {
                  const dup = { ...original, id: crypto.randomUUID(), title: `${original.title} (copy)`, updatedAt: new Date().toISOString() };
                  lib.addPlaylist(dup);
